...
ASPICE 양방향 추적성 및 트레이스 매트릭스 예시

## 양방향 추적성(Bidirectional Traceability)이란?

추적성이란 각 개발 단계의 산출물들이 서로 어떻게 연결되어 있는지 그 관계를 계속 추적할 수 있는 능력을 말한다. 여기에 'Bidirectional' 이란 수식어가 붙는 이유는 다음과 같다.

- Forward Traceability: "그래서 이 요구사항은 어떻게 구현됐지?"
    - 목적: 특정 요구사항 하나가 누락되지 않고 설계, 코드, 테스트까지 모두 반영됐는지 확인하기 위함
    - 예시 질문: "사용자 부재 시 PC를 잠그라는 요구사항(SW-001)은 어떤 코드에서 구현됐고, 어떻게 테스트했나요?"
- Backward Traceability: "도대체 이 코드는 왜 있는거지?"
    - 목적: 코드나 테스트 케이스가 임의로 만들어진 것이 아니라, 명확한 상위 요구사항에 근거하여 만들어졌음을 증명하기 위함
    - 예시 질문: "소스 코드에 있는 `Check_User_Absence_Timer()`라는 함수는 어떤 요구사항 때문에 필요한 건가요?"

이처럼 **'요구사항 -> 코드'** 와 **'코드 -> 요구사항'** 양쪽 방향으로의 연결 관계를 모두 설명할 수 있는 상태를 **'양방향 추적성'** 이 확보됐다고 말한다.

## Traceability Matrix

이러한 복잡한 연결 관계를 한눈에 쉽게 볼 수 있도록 만든 표가 바로 *추적성 매트릭스*이다. 전문적인 툴 없이도 Excel이나, Notion, 간단한 표로 충분히 만들 수 있다.

다음은 이번 프로젝트를 위한 추적성 매트릭스의 예시이다.

| 요구사항 ID | 요구사항 내용                | 설계 (AUTOSAR 모듈)                                      | 구현 (Code File)                               | 검증 (Test Case ID) |
| ------- | ---------------------- | ---------------------------------------------------- | -------------------------------------------- | ----------------- |
| SW-001: | :사용자 부재 시 30초 후 PC 잠금: | `App_PresenceLogic` (SW-C)<br>`App_PcControl` (SW-C) | `app_presence_logic.c`<br>`app_pc_control.c` | TC_SYS_OO1        |
| SW-003  | UART로 감지 상태/ 거리 출력     | `BSW_UartManager`<br>`MCAL_UartDriver`               | `bsw_uart_mgr.c`<br>`mcal_uart.c`            | TC_IF_001         |
| SW-004  | SPI 드라이버 1Mbps 지원      | `MCAL_SpiDriver`                                     | `mcal_spi.c`                                 | TC_SPI_001        |
| SW-005  | LED로 시스템 동작 상태 표시      | `MCAL_DioDriver`                                     | `mcal_dio.c`                                 | TC_DIO_001        |
이 표를 활용하는 방법:
- 정방향 추적: "SW-003 요구사항은 어떻게 구현됐지?"
    - 표의 `SW-003` 행을 따라가면, `BSW_UartManager`와 `MCAL_UartDriver` 모듈로 설계됐고, `bsw_uart_mgr.c`와 `mcal_uart.c` 파일에 코드가 있으며 `TC_IF_001` 테스트 케이스로 검증한다는 것을 한 번에 알 수 있습니다.
- 역방향 추적: `mcal_dio.c` 파일은 왜 만들었지?
    - 표에서 `mcal_dio.c`를 찾으면 이것이 `SW-005` 요구사항("LED로 시스템 상태 표시")을 구현하기 위해 만들어졌다는 것을 역으로 추적할 수 있습니다.

이처럼 간단한 표 하나만으로 프로젝트의 모든 개발 산출물이 어떻게 유기적으로 연결되어 있는지 명확하게 보여줄 수 있으며, 이것이 바로 ASPICE가 요구하는 체계적인 개발 프로세스를 실천하고 있다는 증거가 된다!